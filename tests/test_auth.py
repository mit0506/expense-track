...
def test_register_duplicate_username(client):
    client.post(
        '/register', data={'username': 'dupuser', 'password': 'password123'})
    client.get('/logout')
    resp = client.post('/register', data={
        'username': 'dupuser',
...
def test_login_success(client):
    client.post(
        '/register', data={'username': 'loginuser', 'password': 'password123'})
    client.get('/logout')
...
def test_login_wrong_password(client):
    client.post(
        '/register', data={'username': 'wrongpw', 'password': 'password123'})
    client.get('/logout')
    resp = client.post('/login', data={
        'username': 'wrongpw',
        'password': 'wrongpass',
...
def test_profile_update(auth_client):
    ...
    budget = CategoryBudget.query.filter_by(
        user_id=user.id, category='Food').first()
